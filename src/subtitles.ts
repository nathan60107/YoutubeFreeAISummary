...
export type SubtitleSegment = {
  ...
  start: number;
  /** Duration in seconds (may be 0 when scraped from the transcript panel). */
  duration: number;
...
export type SubtitleResult = {
  ...
  source: "intercept-timedtext" | "transcript-panel";
};
...
function getCaptionTracks(resp: YtPlayerResponse | undefined): YtCaptionTrack[] {
 ...
 */
function pickTrack(tracks: YtCaptionTrack[], preferredLangs: string[]): YtCaptionTrack | undefined {
  if(tracks.length === 0)
    return undefined;
...
function parseJson3(data: Json3Response): SubtitleSegment[] {
...
function toTimedText(segments: SubtitleSegment[]): string {
